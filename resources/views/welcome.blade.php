    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        <div class="flex justify-between items-end mb-12">
            ...
            <div class="mb-8 flex gap-4 justify-center">
                <a href="/"
                    class="p-3 border rounded-xl hover:bg-white hover:shadow-md transition {{ request('category') ? '' : 'bg-indigo-600 text-white' }}">Semua
                    Kategori</a>
                @foreach ($categories as $cat)
                    <a href="/?category={{ $cat->slug }}"
                        class="p-3 border rounded-xl hover:bg-white hover:shadow-md transition {{ request('category') == $cat->slug ? 'bg-indigo-600 text-white' : '' }}">
                        {{ $cat->name }}
                    </a>
                @endforeach
            </div>
